...
oot_channel_id_list = [
    "708146834752929813", #trivia addicts
    "705421948171190343", #nightmare trivia
...
class Bot(discord.Client):
    ...
    def __init__(self, answer_scores):
        ...
        # embed creation
        self.embed=discord.Embed(title="**__SHAIK ASIF__**", description="**Web Searching** :spy:")
        ...
        self.embed.set_footer(text=f"SHAIK ASIF#4945",\
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Game(name='Fetching HQ Answers||+help'))
    ...
    async def on_message(self, message):
        ...
        if message.content.lower() == "+hq":
            ...
            if '719553205880619081' in [role.fetch for role in message.author.roles]:
                self.embed_msg = None
                await self.clear_results()
                await self.update_embeds()
                self.embed_msg = \
        ...
        if message.content.startswith('+help'):
          ...
          if BOT_OWNER_ROLE in [role.name for role in message.author.roles]:
           ...
           embed.add_field(name="when Question come put command", value=" `+hq` **is command work for all support game except**\n**`+j` is command of jeetoh**\n**`+f` is command for filpkart**\n\n**use cmd! in particular channels**\n\n**FOR MORE INFO CONTACT TO SHAIK ASIF#4945**", inline=False)
...
def bot_with_cyclic_update_process(update_event, answer_scores):
    ...
    loop.create_task(bot.start('NTQyNjUwNzMzNjg2ODE2Nzc5.XuNNhw.nhtZFU_C7xSTVZ6kagCFsRkGx08'))
    loop.run_forever()
...
def selfbot_process(update_event, answer_scores):
    ...
    loop = asyncio.get_event_loop()
    loop.create_task(selfbot.start('mfa.XH_Nnvr-gN6FHXSJcf5e1pUPnG_KXjbBPSObn2MM_DvJ-ZRRr0h2v0MfEy3vILpWAjA66AmlNAwNwQENaxcq',
                                   bot=False))
    loop.run_forever()
